...
const MessageStack = ({isOpen, onClose, setNewMessage}) => {
  ...
  useEffect(() => {
    ...
    const newClient = new Client({
      webSocketFactory: () => new SockJS('http://localhost:8083/ase-websocket'),
      onConnect: () => {
    ...
    <DialogComponent isOpen={isOpen} onClose={onClose}>
      <FullWidthBox p={2}>
        ...
        <Stack spacing={2}>
          ...
          {state.messages && state.messages.slice().reverse().slice(0, 5).map(msg => (
            msg ? <Paper key={msg} elevation={2}>
