  ...
  return (
    <div style={{marginTop: '80px'}}>
      <Paper style={{
        ...
        <Grid className="avatar-username" container spacing={2} justifyContent="center" alignItems="center">
          <Grid item container xs={1} justifyContent="center" alignItems="center" flexDirection="row" gap="5px">
            <Avatar alt={`${visitedUserInfo.username}'s Avatar`} src={avatar}
        ...
        <Grid className="follow" container spacing={10} justifyContent="center" alignItems="center">
          <Grid item container justifyContent="center" alignItems="center" flexDirection="column" xs={1}>
            ...
            <Typography onClick={() => handleFollowingsList()} fontSize={16} color="primary" style={{
              paddingTop: '3%',
              paddingBottom: '0%',
              cursor: 'pointer'
            }}> {visitedUserInfo.followings === "" ? 0 : visitedUserInfo.followings.split(',').length} </Typography>
          </Grid>
          <Grid item xs={1} container justifyContent="center" alignItems="center" flexDirection="column">
            ...
            <Typography onClick={() => handleFansList()} fontSize={16} color="primary" style={{
              paddingTop: '3%',
              paddingBottom: '0%',
              cursor: 'pointer'
            }}>{visitedUserInfo.fans === "" ? 0 : visitedUserInfo.fans.split(',').length} </Typography>
          </Grid>
      ...
      <DialogComponent
        ...
        children={<UserList titleText={"You are following:"} setCardOpen={setfollowingCard} userList={followingsList}
                            logginedUser={logginedUser}/>}
      />
      <DialogComponent
        isOpen={fansCard}
        children={<UserList titleText={"Your followers:"} setCardOpen={setfansCard} userList={fansList}
                            logginedUser={logginedUser}/>}
      />
    </div>
  );
